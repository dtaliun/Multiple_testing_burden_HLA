...
option_list <- list(
	...
	make_option(c("-c", "--covar"), type = "character", default = NULL, help = "(Optional) Path to the PLINK2-compatible covariates file.", metavar = "character"),
	make_option(c("-o", "--output"), type = "character", default = NULL, help = "Prefix for output files.", metavar = "character")
)

opt_parser <- OptionParser(option_list = option_list, description = "This script implements a linear regression.")
...
for (pheno_name in pheno_names) {
	...
	for (i in seq_len(nrow(gt))) {
		...
		fit <- try(
  		...
  		if (inherits(fit, "try-error")) {
			...
			coef_idx <- match("G", rownames(sm$coefficients))
			if (is.na(coef_idx) || coef_idx < 1) {
				results[[i]] <- data.frame(
                                	output = output_prefix,
					pheno = pheno_name,
					allele  = allele,
					...
					maf = maf,
                                	...
                                	se = NA_real_,
                                	p =  NA_real_,
                                	error = "Variant dropped",
                                	stringsAsFactors = FALSE
                        	)
			} else {
				beta <- sm$coefficients[coef_idx, "Estimate"]
				se   <- sm$coefficients[coef_idx, "Std. Error"]
				p    <- sm$coefficients[coef_idx, "Pr(>|t|)"]
	...
	out <- dplyr::bind_rows(results)
    	...
    	write.table(
        	out,
        	file = paste0(output_prefix, ".", pheno_name, ".linear.tsv"),
        	sep = "\t",
        	quote = FALSE,
        	row.names = FALSE
    	)
